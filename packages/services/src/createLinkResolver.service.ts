...
export const createLinkResolver = async (arg: ICreateLinkResolver): Promise<string> => {
  const { DLR_API_KEY, dlrAPIUrl, linkResolver, linkResponses, qualifierPath, responseLinkType = 'all' } = arg;
  const registerQualifierPath = arg.queryString ? arg.queryString : qualifierPath; // TODO: refactor this code
  const params: GS1LinkResolver[] = [constructLinkResolver(linkResolver, linkResponses, registerQualifierPath)];
  try {
    privateAPI.setBearerTokenAuthorizationHeaders(DLR_API_KEY);
